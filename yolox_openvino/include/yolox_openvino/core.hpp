...
namespace yolox_openvino{
    ...
    class AbcYoloX{
        public:
            ...
            virtual std::vector<yolox_openvino::Object> inference(const cv::Mat& frame) = 0;
        protected:
            int input_w_;
            int input_h_;
            float nms_thresh_;
            ...
            float intersection_area(const yolox_openvino::Object& a, const yolox_openvino::Object& b)
            {
                cv::Rect_<float> inter = a.rect & b.rect;
                return inter.area();
            }

            void qsort_descent_inplace(std::vector<yolox_openvino::Object>& faceobjects, int left, int right)
            {
                int i = left;
                int j = right;
            ...
            void qsort_descent_inplace(std::vector<yolox_openvino::Object>& objects)
            {
                if (objects.empty())
                    return;
            ...
            void nms_sorted_bboxes(const std::vector<yolox_openvino::Object>& faceobjects, std::vector<int>& picked, const float nms_threshold)
            {
                picked.clear();
            ...
            void decode_outputs(const float* prob, const std::vector<GridAndStride> grid_strides,
                                std::vector<yolox_openvino::Object>& objects, const float bbox_conf_thresh,
                                const float scale, const int img_w, const int img_h)
            {

                std::vector<yolox_openvino::Object> proposals;
